...
//    (1) Searching each payload for a specific key
//    (2) Saving the topic and the value to this key
//    (3) Summarize/evaluate the stored values
...
#include "C_Coll.hpp"                   // additional code
...
void f1PrintHelptext()
{
 ...
 fprintf(stdout, "Purpose: Collect topic+value of a key over all messages.\n");
 fprintf(stdout, "Author : Karl Hartinger\n");
 fprintf(stdout, "Version: 2021-08-31");
...
{
 bool bRet=true;
 bRet=g_coll.readConfig(pfConf);       // read conf data
 if(g_prt) g_coll.show();              // show config values Coll
 return bRet;
}
...
{
 //======Coll: respond to messages===============================
 g_coll.onMessage(mosq, topic, payload);
...
{
 //======Coll: cleanup before end of program=====================
 g_coll.onExit(mosq, reason);
}
