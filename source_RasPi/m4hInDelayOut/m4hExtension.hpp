...
#include "C_InDelayOut.hpp"            // additional code
//#include "C_InDelayOut_Demo.hpp"     // additional code
...
{
 fprintf(stdout, "\nUsage  : m4hInDelayOut [-h | -q | pf.conf]\n");
 ...
 fprintf(stdout, "Version: 2021-09-06");
...
{
 bool bRet=true;
 bRet=g_indelayout.readConfig(pfConf); // read conf data
 if(g_prt) g_indelayout.show();        // show config values
 return bRet;
}
...
{
 //======InDelayOut: respond to messages===============================
 g_indelayout.onMessage(mosq, topic, payload);
...
{
 //======InDelayOut: cleanup before end of program=====================
 g_indelayout.onExit(mosq, reason);
}
