...
#include "C_Xxx.hpp"                   // additional code
...
{
 g_xxx.readConfig(pfConf);            // read conf data
 if(g_prt) g_xxx.show();              // show config values Xxx
}
...
{
 //======Xxx: respond to messages===============================
 g_xxx.onMessage(mosq, topic, payload);
...
{
 //======Xxx: cleanup before end of program=====================
 g_xxx.onExit(mosq, reason);
}
