   ...
   Message2d m2d, std::string currTop, std::string currPay)
{
 ...
 {//----------action given: says how to interpret the payload---
  ...
  {//---------cpay=delay text-----------------------------------
   ...
   if(!g_utils.str2str2(currPay, sDelay, sText,' ')) return;
   m2d.sText=sText;
   try{
 ...
 g_utils.replaceAll(m2d.topicOut,IDO_PLAHO_TOPIC_IN,currTop);
 ...
 g_utils.replaceAll(m2d.payloadOut,IDO_PLAHO_TOPIC_IN,currTop);
