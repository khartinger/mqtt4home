...
and in the first terminal window the message of the tester. 5.   
...
5. change the file name 'C_Wdog.hpp' to 'C_Wdog.hpp   
   In the Explorer window, right-click on `C_Wdog.hpp`, select "Rename" and enter `C_Wdog.hpp`. 6.   
6. in all files rename 'Wdog', 'Wdog' and 'wdog' to 'Wdog', 'Wdog' and 'wdog'.   
...
* Each line `in:` is split into topic and time text sHMS (by the first space character). Then the string `sHMS` is split into hour, minute and second, the strings are converted to numbers and the number of seconds is calculated.   
  Topic and time are converted into a `WdogIn1` object and hooked into the vector `vIn`.   
